 ...
 * Default datarate
 */
#define LORAWAN_DEFAULT_DATARATE                    DR_5 //= SF7 for mapping
...
static void PrepareTxFrame(uint8_t port) {
	...
	case 2: {
		...
		Hdop = GpsGetHdop();
		Satellites = GpsGetSeenSatellites();
		Accuracy = GpsGetAccuracy();
		if (ret)
		{
			printf("[Debug]: latitude: %f, longitude: %f , altitude: %d \r\n", latitude, longitude, altitude);
			printf("[Debug]: Accuracy: %f , HDOP: %f , Satellites: %d\r\n", Accuracy, Hdop, Satellites);
		}
		else
		{
		...
		if (ret == SUCCESS) {
			AppData[0] = 0x01;
			AppData[1] = 0x88;
			AppData[2] = ((int32_t) (latitude * 100000) >> 24) & 0xFF;
			AppData[3] = ((int32_t) (latitude * 100000) >> 16) & 0xFF;
			AppData[4] = ((int32_t) (latitude * 100000) >> 8) & 0xFF;
			AppData[5] = ((int32_t) (latitude * 100000)) & 0xFF;
			AppData[6] = ((int32_t) (longitude * 100000) >> 24) & 0xFF;
			AppData[7] = ((int32_t) (longitude * 100000) >> 16) & 0xFF;
			AppData[8] = ((int32_t) (longitude * 100000) >> 8) & 0xFF;
			AppData[9] = ((int32_t) (longitude * 100000)) & 0xFF;
			AppData[10] = ((altitude * 100) >> 16) & 0xFF;
			AppData[11] = ((altitude * 100) >> 8) & 0xFF;
			AppData[12] = (altitude * 100) & 0xFF;
			AppData[13] = ((int32_t) (Hdop * 100000) >> 24) & 0xFF;
			AppData[14] = ((int32_t) (Hdop * 100000) >> 16) & 0xFF;
			AppData[15] = ((int32_t) (Hdop * 100000) >> 8) & 0xFF;
			AppData[16] = ((int32_t) (Hdop * 100000)) & 0xFF;
...
/*!
 * \brief Function executed on GpsUpdate Timeout event
 */
static void OnGpsUpdateEvent(void) {
	GpsProcess();
	TimerStart(&GpsTimer);
}
...
#error "Please define a region in the compiler options."
			...
			TimerInit(&GpsTimer, OnGpsUpdateEvent);
			TimerSetValue(&GpsTimer, 5000);
			TimerStart(&GpsTimer);
